...
def init_db():
    ...
    # Existing rows from older deployments: use created_at as the initial last-activity time.
    conn.execute("UPDATE conversations SET updated_at = created_at WHERE updated_at IS NULL OR updated_at = ''")
    conn.commit()
    conn.close()
...
def profile_page():
    ...
    conv_count = db.execute("SELECT COUNT(*) AS c FROM conversations WHERE user_id = ?", (session["user_id"],)).fetchone()["c"]
    msg_count = db.execute(
        "SELECT COUNT(*) AS c FROM messages m JOIN conversations c ON m.conversation_id = c.id WHERE c.user_id = ?",
        (session["user_id"],)
    ).fetchone()["c"]
    return render_template("profile.html", user=user, username=session.get("username"), conv_count=conv_count, msg_count=msg_count)
...
def ask_ai(question, recent_messages, pdf_context="", image_path=None, web_context="", model_pref="fast", file_context="", memory_context=""):
...
USER MEMORY (only use when relevant):
{memory_context}
...
def _image_prompt(prompt, edit=False):
    """Turn a short user request into a strict image instruction.

    The model is told to preserve the exact requested subject instead of
    improvising a different person/object/scene. Pollinations' current API
    also supports prompt enhancement, but we keep these instructions in the
    app so the behavior is consistent across models.
    """
    p = (prompt or "").strip()
    if edit:
        return (
            "Edit the provided image according to the user's instruction. "
            "Keep the main subject, identity, pose, composition and all details "
            "that the user did not ask to change. Make ONLY the requested changes. "
            "Do not replace the subject with a different person or object. "
            f"User's edit instruction: {p}"
        )
    return (
        "Create exactly the image the user requested. Follow every named subject, "
        "person, place, object, action, clothing, setting and style literally. "
        "Do not substitute a different person, celebrity, animal, object or scene. "
        "If a real public figure is named, depict that named person accurately in "
        "a photorealistic editorial-style scene while following the request. "
        "Do not add unrelated subjects or change the requested setting. "
        f"User's image request: {p}"
    )


def _save_image_bytes(data, content_type="image/png"):
    if not data:
        return None
    ext = (content_type or "image/png").split("/")[-1].split(";")[0].lower()
    if ext == "jpeg":
        ext = "jpg"
...
def _extract_pollinations_image(data):
    """Extract an image from the OpenAI-compatible Pollinations response."""
    ...
    if b64:
        try:
            return _save_image_bytes(base64.b64decode(b64), "image/png")
        except Exception:
            return None
    url = item.get("url")
    if url:
        try:
            ...
            if r.status_code == 200 and r.content:
                return _save_image_bytes(r.content, r.headers.get("Content-Type", "image/png"))
        except Exception:
            pass
    return None
...
def generate_image(prompt):
    """Generate a new image. Pollinations is primary; Gemini is fallback."""
    improved_prompt = _image_prompt(prompt, edit=False)

    # Current Pollinations API. Keep the secret key server-side in Render.
    if POLLINATIONS_API_KEY:
        try:
            ...
            if response.ok:
                filename = _extract_pollinations_image(response.json())
                if filename:
                    return filename, None
            else:
                print("POLLINATIONS IMAGE ERROR:", response.status_code, response.text[:500])
        except Exception as e:
            print("POLLINATIONS IMAGE REQUEST ERROR:", e)

    # Legacy public endpoint fallback. It is kept for deployments that still
    # have access to the older endpoint; the current API is preferred above.
    try:
        encoded_prompt = requests.utils.quote(improved_prompt)
        seed = uuid.uuid4().int % 1_000_000
        url = (
            f"https://image.pollinations.ai/prompt/{encoded_prompt}"
            f"?model={requests.utils.quote(POLLINATIONS_IMAGE_MODEL)}"
            f"&width=1024&height=1024&seed={seed}&nologo=true&enhance=true"
        )
        response = requests.get(url, timeout=90)
        content_type = response.headers.get("Content-Type", "")
        if response.status_code == 200 and response.content and content_type.lower().startswith("image/"):
            filename = _save_image_bytes(response.content, content_type)
            if filename:
                return filename, None
    except Exception as e:
        print("LEGACY POLLINATIONS IMAGE ERROR:", e)

    # Gemini fallback if it is configured and available.
    if not GEMINI_API_KEY:
        return None, "Image generation is temporarily unavailable. Check the Pollinations API key on Render."
    ...
    try:
        response = requests.post(
            ...
            headers={"x-goog-api-key": GEMINI_API_KEY, "Content-Type": "application/json"},
            json={
                "contents": [{"role": "user", "parts": [{"text": improved_prompt}]}],
        ...
        for part in data["candidates"][0]["content"]["parts"]:
            ...
            if inline:
                ...
                filename = _save_image_bytes(base64.b64decode(b64_data), mime_type)
                if filename:
                    return filename, None
...
def edit_image(image_path, instruction):
    """Edit an existing generated image using the user's plain-text instruction."""
    if not image_path or not os.path.exists(image_path):
        return None, "I couldn't find the previous image to edit. Generate an image first."
    if not POLLINATIONS_API_KEY:
        return None, "Image editing needs a Pollinations API key on Render."
    ...
    try:
        with open(image_path, "rb") as f:
            response = requests.post(
                ...
                files={"image": (os.path.basename(image_path), f, mimetypes.guess_type(image_path)[0] or "image/png")},
                data={
                    "prompt": _image_prompt(instruction, edit=True),
                    "model": os.getenv("POLLINATIONS_EDIT_MODEL", "kontext"),
                    "size": "1024x1024",
                },
                timeout=120,
            )
        if not response.ok:
            print("POLLINATIONS IMAGE EDIT ERROR:", response.status_code, response.text[:700])
            return None, "I couldn't edit that image right now. Please try the change again."
        filename = _extract_pollinations_image(response.json())
        if filename:
            return filename, None
        return None, "I couldn't read the edited image returned by the image service."
    except Exception as e:
        print("POLLINATIONS IMAGE EDIT REQUEST ERROR:", e)
        return None, "Image editing is temporarily unavailable — please try again."


def looks_like_image_edit(text):
    """Conservative detector so ordinary new image prompts aren't edited."""
    q = (text or "").lower().strip()
    edit_phrases = (
        "change ", "edit ", "modify ", "replace ", "remove ", "add ",
        "make the ", "make it ", "turn the ", "turn it ", "put ",
        "move ", "swap ", "fix ", "adjust ", "increase ", "decrease ",
        "background", "sky", "color", "colour", "lighting", "hair", "clothes"
    )
    return any(q.startswith(p) for p in edit_phrases)
...
def api_list_conversations():
    db = get_db()
    convs = db.execute(
        "SELECT id, title, created_at, updated_at FROM conversations WHERE user_id = ? ORDER BY updated_at DESC, created_at DESC",
        (session["user_id"],)
    ).fetchall()
...
def api_chat():
    ...
    force_search = request.form.get("web_search") == "true" and request.form.get("cap_web_search", "true") == "true"
    force_image_gen = request.form.get("image_gen") == "true" and request.form.get("cap_image_gen", "true") == "true"
    files_enabled = request.form.get("cap_files", "true") == "true"
    memory_enabled = request.form.get("cap_memory", "true") == "true"
    memory_context = request.form.get("memory_context", "")[:12000] if memory_enabled else ""
    model_pref = request.form.get("model", "fast")
    file = request.files.get("file") if files_enabled else None
    ...
    if not conv_id:
        ...
        db.execute(
            ...
            (conv_id, session["user_id"], title, datetime.now().isoformat(), datetime.now().isoformat())
        )
    else:
        conv = db.execute(
            "SELECT id FROM conversations WHERE id = ? AND user_id = ?", (conv_id, session["user_id"])
    ...
    # Editing is intentionally automatic: after Novara has generated an image,
    # the user can simply type "make the sky blue" without toggling anything.
    if question and (force_image_gen or (looks_like_image_edit(question) and conv_id)):
        previous_image_path = None
        image_edit_mode = False
        if conv_id and looks_like_image_edit(question):
            previous_image = db.execute(
                "SELECT attachment_path FROM messages WHERE conversation_id = ? AND role = 'assistant' AND attachment_type = 'image' AND attachment_path IS NOT NULL ORDER BY created_at DESC LIMIT 1",
                (conv_id,)
            ).fetchone()
            if previous_image and previous_image["attachment_path"]:
                candidate = os.path.join(UPLOAD_FOLDER, previous_image["attachment_path"])
                if os.path.exists(candidate):
                    previous_image_path = candidate
                    image_edit_mode = True

        if image_edit_mode:
            image_filename, gen_error = edit_image(previous_image_path, question)
        else:
        ...
        reply_text = "I've updated the image." if image_edit_mode else "Here's what I created:"
        db.execute(
            "INSERT INTO messages (id, conversation_id, role, text, attachment_path, attachment_type, sources, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?)",
    ...
    answer = ask_ai(effective_question, recent_messages, pdf_context, image_path_for_ai, web_context, model_pref, file_text_context, memory_context)
