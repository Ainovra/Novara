...
VIDEO_MAX_MB = int(os.getenv("VIDEO_MAX_MB", "15"))  # Gemini's inline (non-Files-API) request works reliably up to roughly this size
...
def auth_google_callback():
    try:
        ...
        print("GOOGLE OAUTH ERROR (token/userinfo):", e)
    ...
    try:
        ...
        # New Google user — pick a username that won't collide with an
        # existing account (e.g. someone who already signed up with the
        # same display name via email/password).
        candidate = name
        suffix = 0
        ...
        db.execute(
            ...
            (user_id, candidate, email, google_id, datetime.now().isoformat())
        )
        db.commit()
        session["user_id"] = user_id
        ...
        print("GOOGLE OAUTH ERROR (database):", e)
        try:
            db.rollback()
        except Exception:
            pass
...
def ask_ai(question, recent_messages, pdf_context="", image_path=None, web_context="", model_pref="fast", video_path=None):
...
"""
    ...
    has_video = video_path and os.path.exists(video_path) and \
        (mimetypes.guess_type(video_path)[0] or "").startswith("video/")
    ...
    # Photos/videos need Gemini's multimodal support — Groq's free tier is
    # text-only, so any message with media attached always goes to Gemini
    # regardless of the Fast/Thinking/Omega choice.
    if GROQ_API_KEY and not has_media:
    ...
    call_timeout = 120 if has_video else 60

    text, error = call_gemini(parts, model=active_model, timeout=call_timeout)
...
def api_chat():
    ...
    if video_too_large:
        answer = (
            f"That video is a bit too large for me to analyze right now (over {VIDEO_MAX_MB}MB). "
            f"I've saved it as an attachment, but I can't watch/describe it — try a shorter clip or a lower-resolution export if you'd like me to look at it."
        )
    else:
        answer = ask_ai(
            effective_question, recent_messages, pdf_context, image_path_for_ai,
            web_context, model_pref, video_path=video_path_for_ai
        )
