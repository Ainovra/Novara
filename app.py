...
POLLINATIONS_IMAGE_MODEL = os.getenv("POLLINATIONS_IMAGE_MODEL", "nanobanana-2")
...
def _image_prompt(prompt, edit=False):
    """Build a precise image instruction without inventing unrelated subjects."""
    p = (prompt or "").strip()
    if edit:
        return (
            "Edit ONLY the provided reference image according to the user's instruction. "
            "Preserve the identity/appearance of the existing subject, composition, camera angle, "
            "clothing, and all unchanged details. Do not replace the person with another person. "
            "Do not create a new scene unless explicitly requested. "
            f"USER EDIT: {p}"
        )
    return (
        "Generate one photorealistic image that follows the user's request literally. "
        "The named subject is the primary subject. Preserve every explicitly named person, "
        "place, object, sport, action, clothing, and setting. Never substitute a different person, "
        "animal, object, or location. Do not add unrelated people or objects. "
        "If a real person is named, treat that exact named person as the requested subject; "
        "do not replace them with a generic-looking person. Use realistic anatomy, natural lighting, "
        "and a believable environment. Do not add text, logos, watermarks, captions, or borders "
...
def generate_image(prompt):
    ...
    # Do not silently switch to the legacy image.pollinations.ai endpoint.
    # It can produce a different model/result than the authenticated API above.
