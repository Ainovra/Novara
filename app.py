...
IMAGE_MODEL = os.getenv("IMAGE_MODEL", "gemini-3.5-flash-image")
...
GROQ_OMEGA_MODEL = os.getenv("GROQ_OMEGA_MODEL", "deepseek-r1-distill-llama-70b")
...
def ask_ai(question, recent_messages, pdf_context="", image_path=None, web_context="", model_pref="fast"):
...
def generate_image(prompt):
    """
    Calls a Gemini image-generation-capable model and saves the returned
    image to the uploads folder. Returns (filename, error_message).
    If your API key doesn't have access to IMAGE_MODEL, set a different
    model name via the IMAGE_MODEL environment variable on Render.
    """
    if not GEMINI_API_KEY:
        return None, "GEMINI_API_KEY not set on server."
    ...
    try:
        ...
        return None, "The model didn't return an image for this prompt."
...
def api_chat():
    ...
    answer = ask_ai(effective_question, recent_messages, pdf_context, image_path_for_ai, web_context, model_pref)
