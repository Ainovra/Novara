...
## 10. New: Fast v1 / Thinking v1 / Omega v1 model picker

A small toggle now sits at the top of the chat with three options: **⚡ Fast v1**, **💭 Thinking v1**, **🧠 Omega v1**.

- **Fast v1** — the everyday model (`gemini-3.5-flash`), confirmed working, quick replies.
- **Thinking v1** — a more careful model (env var `THINKING_MODEL`), told to reason step by step before answering.
- **Omega v1** — the deepest-reasoning option (env var `OMEGA_MODEL`), told to double-check its logic — meant for code, debugging, and tough multi-step problems.

Your choice is remembered in the browser and applies to your next message.

**Being fully honest about this:** all three are general-purpose Gemini models, not a specialized "coding-only" model — any of them can write code, but a bigger/more capable model generally does better on hard problems. Right now, **`THINKING_MODEL` and `OMEGA_MODEL` both default to the same guessed model name** (`gemini-3.5-pro`), since only `gemini-3.5-flash` (Fast) has been confirmed to work on your API key so far. That means Thinking v1 and Omega v1 will currently behave almost identically except for the instruction wording — until you check Google AI Studio for what other model names your key actually has access to, and set `THINKING_MODEL` / `OMEGA_MODEL` independently on Render (Environment tab) to two genuinely different models if available. If a model name isn't available on your plan, Novara automatically falls back to Fast v1 rather than failing.

## 11. New: Groq — free, genuinely distinct models for Fast / Thinking / Omega

The Gemini-only setup above had a real limitation: Thinking and Omega were both guesses at the same model. **Groq** fixes that — it's a separate, genuinely free API (no credit card) that hosts real, distinct models, including an actual reasoning model.

| Tier | Groq model (default) | Why |
|---|---|---|
| Fast v1 | `llama-3.1-8b-instant` | Very generous free limit (14,400 requests/day) |
| Thinking v1 | `llama-3.3-70b-versatile` | Larger, better quality |
| Omega v1 | `deepseek-r1-distill-llama-70b` | A genuine reasoning model — it actually works through problems step by step internally, not just "told to think" |

**Honest limitation:** Groq's free tier is text-only — no photo/image understanding. So Novara automatically keeps using Gemini for any message that has an image attached, no matter which of the three tiers is selected. Groq only powers plain text conversations.

### Setup

1. Go to [console.groq.com](https://console.groq.com), sign up (Google/GitHub login, no credit card)
2. Create an API key (left menu → API Keys → Create)
3. On Render, go to your service → **Environment** tab, add:
...
That's it — Fast/Thinking/Omega now hit three genuinely different models automatically. If you ever want to change which exact Groq model backs each tier (their model names shift occasionally), you can override any of them without touching code:

| Key | Default |
|---|---|
| `GROQ_FAST_MODEL` | `llama-3.1-8b-instant` |
| `GROQ_THINKING_MODEL` | `llama-3.3-70b-versatile` |
| `GROQ_OMEGA_MODEL` | `deepseek-r1-distill-llama-70b` |

If `GROQ_API_KEY` isn't set, or a Groq request fails for any reason (rate limit, model renamed, etc.), Novara automatically falls back to the old Gemini-based logic — so this is safe to try without breaking anything.

## 12. Fixed: image generation quota error + mic not listening

**Image generation** now uses **Pollinations.ai** instead of Gemini. The Gemini error you saw (`limit: 0`) meant Gemini's image model isn't available on the free tier at all — no retrying or waiting fixes that, it's paid-only. Pollinations is genuinely free and needs **no API key or signup** — nothing to configure, it just works. Gemini is kept as a silent fallback only, in case you ever add billing to your Google account.

**Microphone / speech-to-text** — the real bug was that errors (permission denied, no speech detected, unsupported language, etc.) were happening silently — the mic button would just stop "listening" with no explanation, which is why it looked like it "wasn't hearing you." Now:
- If your phone reports a language the speech engine doesn't support, it automatically retries in English right away (no need to tap the mic twice)
- Any other issue (blocked permission, no mic found, no speech heard, no internet) now shows a short message under the input box for a few seconds, so you actually know what happened instead of it just failing quietly

No new setup needed for either fix — just update the files below.

## 13. New: model picker moved next to attach button + native Camera/Photos/Files menu

**Model picker** — Fast/Thinking/Omega moved out of the top bar and into the input row itself, right next to the attach (📎) button. It's now a small button showing the current mode (e.g. "⚡ Fast") — tap it to open a dropdown with all three options and a one-line description of each. Defaults to Fast, and your choice is still remembered.

**Attach button** — tapping 📎 now shows a small menu first, like most modern AI apps:
- **📷 Camera** — opens your phone's camera directly (not a general picker)
- **🖼️ Photos & Videos** — opens your gallery
- **📄 Files** — opens the general file browser (PDFs, docs, zip, etc.)

Each option opens the right native picker immediately instead of one generic "choose a file" dialog. No setup needed — this is all in the frontend.

## 14. Fixed: mic showed "listening" but never caught anything

The real bug: tapping the mic was requesting the microphone **twice at the same time** — once directly (for the live waveform animation) and once internally by the browser's own speech recognition. On several Android devices, that conflict lets the mic indicator turn on while the actual recognizer never receives any audio — so it looked "open" but nothing was ever heard, silently.

Fixed by only letting speech recognition manage the microphone itself. The trade-off: the waveform next to the mic icon is now a smooth pulse animation rather than reacting to your literal voice volume — but the mic now actually captures what you say.
