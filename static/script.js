...
// ===== Model picker (Fast / Thinking / Omega) =====
...
function startWaveform(stream) {
  ...
  function draw() {
    ...
    waveBars.forEach((bar, i) => {
      // Sample a few different frequency bins so bars move independently,
      // based on the person's actual voice volume — not a fake/generic animation.
      const value = dataArray[i * 4] || 0;
      const scale = 0.25 + (value / 255) * 1.4;
...
if ("webkitSpeechRecognition" in window || "SpeechRecognition" in window) {
  ...
  recognition.onerror = (e) => {
    ...
    // browser's speech engine doesn't actually support — retry once in
    // plain English instead of failing every time with no explanation.
  ...
  if (micBtn) {
    micBtn.onclick = async () => {
      ...
      // Ask for mic permission FIRST — starting recognition before the
      // permission prompt resolves is what was causing silent failures.
      try {
        micStream = await navigator.mediaDevices.getUserMedia({ audio: true });
        startWaveform(micStream);
      } catch (err) {
        // No waveform if permission is denied/unavailable — we still try
        // recognition below, since some browsers manage their own mic access.
      }
      ...
      try {
        ...
        // recognition was already running or in a bad state — reset the UI
        // instead of leaving the button stuck on "listening".
        micBtn.classList.remove("listening");
        stopWaveform();
      }
    };
