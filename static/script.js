...
function updateActionButton(){
  ...
  const listening = sendBtn.classList.contains("listening");
  sendBtn.classList.toggle("show-send", hasInput && !listening);
  sendBtn.classList.toggle("show-mic", !hasInput && !listening);
  sendBtn.disabled = listening ? false : (!hasInput && !(novaraSettings.voice_assistant && voiceSupported));
  sendBtn.setAttribute("aria-label", listening ? "Stop voice input" : (hasInput ? "Send message" : "Voice assistant"));
  sendBtn.title = listening ? "Stop listening" : (hasInput ? "Send message" : "Talk to Novara");
}
...
if (voiceSupported) {
  ...
  if (micBtn) {
    micBtn.onclick = () => {
      ...
      // One beautiful action button: send when there is text/file,
      // otherwise it becomes the voice-assistant button.
      if (messageInput.value.trim() || pendingFile) {
        sendMessage();
        return;
      }
  ...
  // Keep the button available for typed messages on browsers without
  // SpeechRecognition; it is disabled only while the input is empty.
...
async function sendMessage() {
  ...
  try {
    ...
    if (data.error) {
      ...
      if (novaraSettings.voice_assistant) speakNovara(data.reply);
      loadConversations();
    }
  } catch (e) {
    typingEl.remove();
...
// ===== Inline markdown (bold / italic / tables) for plain-text segments =====
...
function buildTableHtml(tableLines) {
  ...
  headerCells.forEach(c => { html += `<th>${renderInlineMarkdown(escapeHtml(c))}</th>`; });
  html += "</tr></thead><tbody>";
  bodyRows.forEach(row => {
    html += "<tr>";
    row.forEach(c => { html += `<td>${renderInlineMarkdown(escapeHtml(c))}</td>`; });
    html += "</tr>";
  });
...
// Renders a plain-text segment (no fenced code in it): detects markdown
// tables line-by-line and builds real <table> markup for them, and applies
// bold/italic to everything else. Everything is HTML-escaped first.
function renderTextSegment(raw) {
  ...
  while (i < lines.length) {
    ...
    const looksLikeSeparator = /^\s*\|?(\s*:?-+:?\s*\|)+\s*:?-*:?\s*\|?\s*$/.test(nextLine);
    ...
    if (looksLikeRow && looksLikeSeparator) {
      ...
      html += renderInlineMarkdown(escapeHtml(line)) + (i < lines.length - 1 ? "\n" : "");
      i++;
    }
  }
  return html;
...
function renderBubbleContent(text) {
  ...
  while ((match = codeBlockRegex.exec(text)) !== null) {
    ...
    if (match.index > lastIndex) {
      html += renderTextSegment(text.slice(lastIndex, match.index));
    }
  ...
  if (lastIndex < text.length) {
    html += renderTextSegment(text.slice(lastIndex));
  }
...
function renderMessage(m, attachmentUrl) {  const el = document.createElement("div");
  ...
  if (attachmentUrl && m.attachment_type === "image") {
    attachmentHtml = `<div class="msg-attachment generated-image-wrap"><img src="${attachmentUrl}" loading="lazy" alt="Novara generated image">${m.role === "assistant" ? '<div class="generated-image-hint">Type a change below to edit this image.</div>' : ''}</div>`;
...
function wireActionBar(el, m) {
  ...
  bar.querySelector('[data-action="play"]').onclick = (e) => {
    ...
    utter.volume = Number(novaraSettings.volume ?? 1);
    const voices = getVoiceList();
    if (novaraSettings.voice && voices[Number(novaraSettings.voice)]) utter.voice = voices[Number(novaraSettings.voice)];
...
// ===== Apply capability states to the chat UI =====
if (webSearchToggle) { webSearchToggle.classList.toggle("disabled", !novaraSettings.web_search); webSearchToggle.setAttribute("aria-disabled", String(!novaraSettings.web_search)); }
if (imageGenToggle) { imageGenToggle.classList.toggle("disabled", !novaraSettings.image_gen); imageGenToggle.setAttribute("aria-disabled", String(!novaraSettings.image_gen)); }
if (attachBtn) { attachBtn.classList.toggle("disabled", !novaraSettings.files); attachBtn.setAttribute("aria-disabled", String(!novaraSettings.files)); }
if (micBtn) { micBtn.classList.toggle("disabled", !novaraSettings.voice_assistant); }
updateActionButton();
